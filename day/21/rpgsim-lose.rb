...
arm_best_cost[1] = 
  ...
  .transpose.map { |r| r.compact.max }
arm_best_cost[2] = 
  (arm_total_cost[0] + arm_total_cost[1] + arm_total_cost[2]) \
  .transpose.map { |r| r.compact.max }

puts
puts 'Worst armor costs:'
...
while player['Armor'] >= 0
  ...
  while player['Damage'] > 3
    ...
    if !(player_win <= boss_win)
      puts "Player loses with Arm:#{player['Armor']} Dmg:#{player['Damage']}"
      a = player['Armor']
      d = player['Damage']
      solutions << [a,d]
      [0,1,2].each do |r|
